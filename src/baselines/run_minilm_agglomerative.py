...
from clustering.agglomerative import run_agglomerative
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MiniLM + Agglomerative Clustering.")
...
def main() -> None:
    ...
    embeddings_path = embeddings_dir / "minilm_embeddings.npy"
    metrics_path = output_dir / "minilm_agglomerative_metrics.json"
    predictions_path = output_dir / "minilm_agglomerative_predictions.csv"
    ...
    LOGGER.info(
        "Running AgglomerativeClustering with n_clusters=%d, metric=%s, linkage=%s",
        args.n_clusters,
        args.metric,
        args.linkage,
    )
    cluster_labels, _ = run_agglomerative(
        embeddings,
    ...
    metrics.update(
        {
            "agglomerative_metric": args.metric,
            "agglomerative_linkage": args.linkage,
        }
    )
...
def _load_or_build_embeddings(
    ...
    if embeddings_path.exists() and not args.force:
        LOGGER.info("Reusing cached MiniLM embeddings from: %s", embeddings_path)
        embeddings = load_embeddings(embeddings_path)
        if len(embeddings) == expected_count:
            return embeddings
        LOGGER.warning(
            "Cached MiniLM embeddings have %d rows, but input data has %d rows. "
