...
def main() -> None:
    ...
    _log_preflight_environment(settings)
    if args.quick:
...
def _log_preflight_environment(settings: dict[str, int | float | str | None]) -> None:
    LOGGER.info("Preflight environment check")
    ...
    try:
        ...
        LOGGER.warning("Could not import torch during preflight: %s", exc)

    LOGGER.info("expected train CSV: %s", _path_arg(_resolve_root_path(settings["train_file"])))
    LOGGER.info(
        "expected validation CSV: %s",
        _path_arg(_resolve_root_path(settings["validation_file"])),
    )

    executable_path = Path(sys.executable).resolve()
    root_path = ROOT.resolve()
    executable_parts = {part.lower() for part in executable_path.parts}

    if (".venv" in executable_parts or "venv" in executable_parts) and not _is_relative_to(
        executable_path,
        root_path,
    ):
        LOGGER.warning(
            "Python executable appears to come from a virtual environment outside "
            "the project root. This can cause dependency mismatch: %s",
            executable_path,
        )


def _resolve_settings(args: argparse.Namespace) -> dict[str, int | float | str | None]:
    common = {
    ...
    if args.quick:
        return common | {
            ...
            "max_length": args.max_length or 96,
            "num_workers": 0 if args.num_workers is None else args.num_workers,
            "torch_num_threads": args.torch_num_threads or 1,
            "gradient_accumulation_steps": args.gradient_accumulation_steps or 1,
            "limit_train_samples": args.limit_train_samples or 120,
        }
...
def _prepare_data(settings: dict[str, int | float | str | None]) -> list[StepResult]:
...
def _run_all_baselines(
    ...
    LOGGER.info("Running 8 baseline configuration")
    command = [
        sys.executable,
        "-m",
    ...
    if args.msimcse_model_name:
        ...
        LOGGER.info("No --msimcse_model_name provided; Baseline 5 will be marked as skipped.")
    if args.local_files_only:
        command.append("--local_files_only")
    if args.offline:
        command.append("--offline")
    result = _run_step("run_all_baselines", command)
    _log_baseline_summary(ROOT / "outputs" / "metrics" / "baseline_summary.csv")
    return result


def _train_supcon(settings: dict[str, int | float | str | None]) -> StepResult:
    ...
    command = [
        ...
        "--torch_num_threads",
        str(settings["torch_num_threads"]),
        "--gradient_accumulation_steps",
    ...
    if settings["limit_train_samples"] is not None:
        command.extend(["--limit_train_samples", str(settings["limit_train_samples"])])

    return _run_step("train_supcon_mbert", command)


def _extract_supcon_embeddings(settings: dict[str, int | float | str | None]) -> StepResult:
...
def _run_visualizations(settings: dict[str, int | float | str | None]) -> list[StepResult]:
...
def _load_metric_rows(metrics_dir: Path) -> list[dict]:
    rows = []
    baseline_rows = _load_baseline_summary_rows(metrics_dir)
    metric_files = {
        "tfidf_kmeans": "tfidf_kmeans_metrics.json",
        "tfidf_svd_kmeans": "tfidf_svd_kmeans_metrics.json",
        "minilm_kmeans": "minilm_kmeans_metrics.json",
        "minilm_agglomerative": "minilm_agglomerative_metrics.json",
        "mbert_kmeans": "mbert_kmeans_metrics.json",
        "mbert_dbscan": "mbert_dbscan_metrics.json",
        "labse_kmeans": "labse_kmeans_metrics.json",
    ...
    for model_name, file_name in metric_files.items():
        ...
        if model_name in baseline_rows:
            baseline_row = baseline_rows[model_name]
            for column in SUMMARY_COLUMNS:
                if column in baseline_row:
...
def _log_baseline_summary(summary_path: Path) -> None:
    if not summary_path.exists():
        LOGGER.warning("baseline_summary.csv was not found at: %s", summary_path)
        return

    LOGGER.info("baseline_summary.csv path: %s", summary_path)
    summary = pd.read_csv(summary_path)
    if not {"model_name", "status"}.issubset(summary.columns):
        LOGGER.warning("baseline_summary.csv exists but does not contain expected columns.")
        return
...
def _load_baseline_summary_rows(metrics_dir: Path) -> dict[str, dict]:
    baseline_summary_path = metrics_dir / "baseline_summary.csv"
    ...
    return {
        str(row["model_name"]): row.dropna().to_dict()
        for _, row in summary.iterrows()
    }


def _prepare_data_sample_limit(settings: dict[str, int | float | str | None]) -> int:
...
def _verify_processed_data_files(settings: dict[str, int | float | str | None]) -> None:
...
def _train_file(settings: dict[str, int | float | str | None]) -> Path:
    return resolve_processed_file([settings["train_file"]])


def _validation_file(settings: dict[str, int | float | str | None]) -> Path:
...
def _run_step(
    ...
    if result.returncode != 0:
        ...
        if required:
            if name == "train_supcon_mbert":
                raise RuntimeError(
                    "SupCon training failed. Run train_supcon.py alone with safe "
                    "CPU settings to debug."
                )
